/**
 * ClipFlow Obsidian 本地双脑增量同步 CLI 工具
 * 
 * 使用方法:
...
interface SyncConfig {
  ...
  targetServerUrl: string  // ClipFlow 服务地址，默认 http://localhost:3000
...
async function main() {
  ...
  console.log(`🚀 检测到有 ${uploadQueue.length} 个知识笔记发生更新，准备推流至 ClipFlow 智能体云端...`)
  ...
  try {
    ...
    console.log(`🎉 同步成功！成功将 ${resJson.syncedCount} 条增量知识注入 ClipFlow 企业大脑。`)
    ...
    console.error("❌ 错误: 云端同步网络请求失败，请检查 ClipFlow 服务是否已启动。")
