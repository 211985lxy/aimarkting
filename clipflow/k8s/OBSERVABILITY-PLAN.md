# ClipFlow Observability & Stability Plan
...
### K8s Configuration
- [x] CronJob: SLB IP made configurable via `CLIPFLOW_WEB_HOST` env (note: ECI has no CoreDNS, Service DNS unavailable)
...
- [x] Created SLS Project: `clipflow-logs` (cn-hangzhou)
...
- [ ] TODO: Install ARMS or managed Prometheus addon in ACK to scrape `/api/metrics`
  ...
  - Then create ServiceMonitor targeting `clipflow-web:80/api/metrics` every 30s

### 2.3 Alert Rules - DONE (via CMS)

Created 5 CMS alert rules (all active, contact group: ClipFlow-Alerts):

| Rule ID | Name | Metric | Threshold |
|---------|------|--------|-----------|
| clipflow-eci-cpu-high | ECI CPU使用率过高 | instance_cpu_utilization | Warn >70%, Critical >85% |
| clipflow-eci-memory-high | ECI 内存使用率过高 | instance_memory_utilization | Warn >80%, Critical >90% |
| clipflow-slb-5xx | SLB 5xx错误率飙升 | InstanceStatusCode5xx | Warn >3/s, Critical >10/s |
| clipflow-slb-high-rt | SLB 响应时间过长 | InstanceRt | Warn >5s, Critical >10s |
| clipflow-slb-unhealthy | SLB 后端实例异常 | UnhealthyServerCount | Warn >=1, Critical >=2 |
...
- [ ] `curl http://clipflow-web/api/healthz` returns DB/Redis status
- [ ] `curl http://clipflow-web/api/metrics` returns Prometheus metrics
