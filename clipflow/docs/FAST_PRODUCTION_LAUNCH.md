# ClipFlow 快速上线清单

目标：先上线一个可用版本，覆盖登录、客户项目、AIM 改文案、AIM 短视频内容生产。

## 1. 服务器最低配置

- Ubuntu 22.04 / 24.04
- 2C4G 起步
- Docker + Docker Compose
- Nginx
- 一个已解析到服务器的域名

## 2. 安装基础软件

```bash
apt update && apt upgrade -y
apt install -y git nginx docker.io docker-compose-plugin certbot python3-certbot-nginx
systemctl enable --now docker nginx
```

## 3. 上传代码

```bash
mkdir -p /var/www/clipflow
cd /var/www/clipflow
git clone <your-repo-url> .
cd clipflow
```

如果没有 Git 仓库，先用 `scp` 或服务器面板上传整个项目目录。

## 4. 配置生产环境变量

```bash
cd /var/www/clipflow/clipflow
cp apps/web/.env.production.example apps/web/.env.production
nano apps/web/.env.production
```

必须填写：

```bash
MYSQL_ROOT_PASSWORD
MYSQL_PASSWORD
JWT_SECRET
ADMIN_JWT_SECRET
ADMIN_EMAIL
ADMIN_PASSWORD
THEROUTER_API_KEY 或 DEEPSEEK_API_KEY 或 OPENAI_API_KEY
CRON_SECRET
```

生成随机密钥：

```bash
openssl rand -base64 32
```

## 5. 启动服务

先校验配置：

```bash
cd /var/www/clipflow/clipflow
docker compose --env-file apps/web/.env.production -f docker-compose.prod.yml config >/tmp/clipflow-compose-check.yml
```

本地如果只想用示例文件检查 compose 语法：

```bash
CLIPFLOW_WEB_ENV_FILE=./apps/web/.env.production.example \
docker compose --env-file apps/web/.env.production.example -f docker-compose.prod.yml config >/tmp/clipflow-compose-check.yml
```

```bash
cd /var/www/clipflow/clipflow
docker compose --env-file apps/web/.env.production -f docker-compose.prod.yml up -d --build
```

检查：

```bash
docker ps
curl -I http://127.0.0.1:3000/login
```

## 6. 初始化数据库

```bash
docker compose --env-file apps/web/.env.production -f docker-compose.prod.yml exec web sh -lc "cd apps/web && npx prisma db push && pnpm prisma db seed"
```

如果 seed 不可用，至少先执行：

```bash
docker compose --env-file apps/web/.env.production -f docker-compose.prod.yml exec web sh -lc "cd apps/web && npx prisma db push"
```

## 7. 配置 Nginx

```bash
cp infra/nginx.clipflow.conf /etc/nginx/sites-available/clipflow
nano /etc/nginx/sites-available/clipflow
```

把 `your-domain.com` 改成真实域名。

```bash
ln -sf /etc/nginx/sites-available/clipflow /etc/nginx/sites-enabled/clipflow
nginx -t
systemctl reload nginx
```

## 8. 配置 HTTPS

```bash
certbot --nginx -d your-domain.com
certbot renew --dry-run
```

## 9. 上线验收

打开：

```text
https://your-domain.com/login
https://your-domain.com/admin/login
https://your-domain.com/home
https://your-domain.com/projects
https://your-domain.com/aim
```

必须验证：

- 能注册 / 登录
- 能进入后台
- 能创建客户项目
- AIM 能改文案
- AIM 能生成短视频内容
- 最近内容状态能切换

## 10. 当前不建议上线前继续做的事

- 不继续扩展热点、对标、视频生成、自动发布。
- 不重构全部旧页面。
- 不接复杂自动化。

第一版只验证：客户项目 + 改文案 + 短视频内容生成 + 状态推进。
