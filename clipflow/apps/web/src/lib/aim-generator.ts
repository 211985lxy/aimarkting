...
- 语言专业但易懂
- 适合微信公众号阅读习惯
- 【GEO引用锚点规则】当文章中出现数据性结论、行业趋势断言或统计数字时，在该句末尾以括号追加引用建议，格式为：（数据参考：建议引用[机构名称]相关报告/白皮书）。例如："短视频广告的ROI已超过图文形式3倍以上（数据参考：建议引用QuestMobile或艾瑞咨询短视频营销报告）"。引用建议每篇文章出现1-2处即可，不要过多，且只在有真实数据背书价值的断言处添加，不要在普通描述性句子后添加。`,
