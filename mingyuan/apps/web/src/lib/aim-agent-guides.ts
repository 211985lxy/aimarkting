...
const TOPIC_PLANNING_SKILLS: AimWorkbenchSkill[] = [
  {
    ...
    prompt: "请基于当前会议纪要，生成一份《会议纪要内容资产包》：先整理会议概览、核心矛盾、关键事实和人物/项目关系，再输出选题池、任务清单、采访问题、脚本方向、拍摄清单和可沉淀到知识库的素材。所有结论必须能追溯到会议纪要，缺失信息标注待补充。",
...
export const AIM_AGENT_GUIDES: Record<AimAgentId, AimAgentGuide> = {
  ...
  business_diagnosis: {
    ...
    defaultInstruction: "按灵感选题策划输出：先判断本次选题要参考哪类知识库资料，再识别对标账号/对标内容，围绕热点类、人设类、问题解答类、观点类四类内容主线生成选题池，筛选高潜选题，并确定核心内容方向。不同选题要匹配不同资料：问题解答类优先客户痛点/问答/会议纪要，转化类优先产品卖点/案例/成交记录，人设类优先老板经历/定位素材，热点类优先行业信源/对标动态。只有用户明确要求基于会议纪要，或本次选题素材选中了会议纪要时，才从会议里的真实问题、原话、分歧、案例和下一步动作提炼选题。热点只作为行业线索，必须结合当前账号资料、对标账号、对标文案和资料库内容推荐；缺少依据时标注待补充。",
    quickPrompts: [
