...
7. 不同选题必须匹配不同知识库资料，不要把某一类资料固定当成核心：问题解答类优先客户痛点/客户问答/会议纪要，转化类优先产品卖点/项目案例/成交记录，人设类优先老板经历/定位素材，热点类优先行业信源/对标动态。
8. 只有用户明确要求「基于会议纪要/调用会议纪要」，或本次选题素材明确选中了会议纪要时，才把会议纪要作为主要依据，并从客户原话、真实问题、分歧、案例、顾虑和下一步动作中拆选题。
9. 会议纪要内容资产包路由的输出结构固定为：会议概览、核心矛盾/机会、关键事实与原话、可拍选题池、优先级最高的选题、任务清单、采访问题、脚本/分镜方向、可沉淀知识库素材、待补充信息。选题池要覆盖热点类、人设类、问题解答类、观点类，并说明每条参考了哪类资料。
