...
export function EvidenceDashboard({
  ...
  const hasTopVideos = evidence.topVideos.length > 0
  ...
  return (
    <section className="space-y-3">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {metrics.map((m) => (
          <Card key={m.label} className="p-3 bg-muted/40 border-0">
        ...
        {hasTopVideos && (
          <Card>
            ...
            <Table>
              ...
              <TableBody>
                {evidence.topVideos.slice(0, 10).map((v, i) => (
                  <TableRow key={i}>
                    <TableCell className="text-muted-foreground text-sm">{i + 1}</TableCell>
                    <TableCell className="text-sm max-w-[180px] truncate">{v.title || "—"}</TableCell>
                    <TableCell className="text-sm">{v.views > 0 ? formatCount(v.views) : "-"}</TableCell>
          ...
          <div className="min-w-[420px]">
            ...
            {DAYS.map((day) => (
              <div key={day} className="flex mb-1 items-center">
                <div className="w-7 shrink-0 text-xs text-muted-foreground text-right pr-2">
                  {DAY_LABELS[day]}
                </div>
                {HOURS.map((h) => (
                  <div
                    key={h}
                    className={`flex-1 h-5 rounded-sm mx-px ${cellIntensity(day, h, evidence.postingHeatmap, maxHeat)}`}
                  />
                ))}
              </div>
            ))}
