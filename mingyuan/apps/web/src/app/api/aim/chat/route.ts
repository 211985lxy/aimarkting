...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const knowledgeContext = shouldUseKnowledgeContextForTask(runtimeTask)
    ...
    const styleBlock = shouldUseKnowledgeContextForTask(runtimeTask)
      ? await runAimTraceStep(
          trace,
          "style_profile",
          "风格档案召回",
    ...
    const competitorWatchBlock =
      agentId === "business_diagnosis" && shouldUseMarketViralContextForTask(runtimeTask)
        ? await runAimTraceStep(
            trace,
            "competitor_context",
            "竞品上下文召回",
