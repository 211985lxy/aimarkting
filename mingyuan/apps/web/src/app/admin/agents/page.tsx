...
export default function AdminAgentsPage() {
  ...
  const loadDetail = React.useCallback(async (id: string) => {
    const raw = localStorage.getItem("mingyuan-admin-auth")
    const authToken = raw ? JSON.parse(raw).state?.token || "" : ""
  ...
  React.useEffect(() => {
    ...
    async function loadList() {
      ...
      const payload = await res.json()
      if (cancelled) return
      setTraces(payload.data.traces)
      setStats(payload.data.stats)
      setLoading(false)
      if (!selectedIdRef.current && payload.data.traces[0]) {
        void loadDetail(payload.data.traces[0].id)
      }
    }
